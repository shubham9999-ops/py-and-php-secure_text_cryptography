...
if(isset($_POST["Login"])){
   ...
   $query="select username,pass from admin where id=1;";
   if($q=mysqli_query($connect,$query)){
       $row=mysqli_fetch_row($q);
       $r0=decryptthis($row[0],$key);
       $r1=decryptthis($row[1],$key);
       if($r0==$u&&$r1==$p){
        header("Location:intro.php");
        $python2=`python voice.py`;
        echo $python2;
    }
    else
...
<form id="accesspanel" action="" method="post">
  <h1 id="litheader">CYBER &nbsp ACADEMY</h1>
  <div class="inset">
    <p>
  ...
  <p class="p-container">
    <input type="submit" name="Login" id="go" value="Grant Access">
  </p>
</form>
